...
from rnn_slu.rnn.elman import model
...
#Read the validation file
    ...
    best_f1 = -numpy.inf
    s['clr'] = s['lr']
    f1=open(folder+'/f1_t_lr_costOnEpoch.dat', 'w+')
    f1.close()
	...
	total_cost = 0
        count = 0
        for i in xrange(nsentences*s['ptrain']/100):
		...
		total_cost += rnn.train(word_batch, label_last_word, s['clr'])
                ...
                rnn.normalize()
            if s['verbose']:
                print '[learning] epoch %i >> %2.2f%%'%(e,(i+1)*100./(nsentences*s['ptrain']/100)),'completed in %.2f (sec) <<\r'%(time.time()-tic),
                sys.stdout.flush()
        print ''
        ...
        f1 = open(folder+'/f1_t_lr_costOnEpoch.dat',"a") ## file open in appending mode i.e 'a'
        print >> f1, '',e, res_valid['f1'],(time.time()-tic),s['clr'],total_cost/count
        f1.close()
